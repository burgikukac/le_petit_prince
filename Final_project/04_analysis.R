...
load(file = "./Final_project/final_tidy_1990.RData") # loading the tidy text

download_this <-
  read_feather("./Final_project/download_this.feather") # loading the metadata

metadata <- download_this %>%
  select(gutenberg_id, gutenberg_author_id, year)  # filtering out the unnecessary metadata
...
senti_by_book <- final_tidy_1990 %>%  # calculating average sentiment score AFINN
...
senti_by_book_with_meta <- senti_by_book %>%  # joining the metadata (year)
  left_join(metadata)

### sentiments 2: nrc (anger, trust, fear, etc.)
sentiments_nrc <- get_sentiments("nrc")

senti_by_nrc_with_meta <- final_tidy_1990 %>%  # counting each sentiment 
...
sentiments_nrc_alter <-
  sentiments_nrc %>%  # custom sentiment dict. based on only positive and negative
  filter(sentiment %in% c("positive", "negative")) %>%
  mutate(score = ifelse(sentiment == "positive", 1,-1))

senti_nrc_pos_neg <- final_tidy_1990 %>%  # nrc sentiment score with the custom dict.
...
senti_together <- senti_by_book_with_meta %>%  # afinn and nrc together
