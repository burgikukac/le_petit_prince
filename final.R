...
## we have 'looked' that contains the possible booklist with the metadata
## now we download the books

download_this <- looked %>% 
  filter(year <1990)

#final_raw_1990 <- gutenberg_download(as.vector(download_this$gutenberg_id))

## we have final_raw_1990

final_tidy_1990 <- final_raw_1990 %>% 
  unnest_tokens(word, text)
save(final_tidy_1990, file ="final_tidy_1990.RData")
